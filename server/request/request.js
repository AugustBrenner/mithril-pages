...
module.exports = function(Promise) {
	...
	var request = function(url, args, resolve, reject) {
		...
		var assumeJSON = (args.serialize == null || args.serialize === JSON.serialize)
		//=======
	...
	var modules = {
		...
		jsonp: makeRequest(function(url, args, resolve, reject) {
			...
			var callbackName = args.callbackName || "_mithril_" + Math.round(Math.random() * 1e16)

			url = interpolate(url, args.data, true)
			var script_src = url + (url.indexOf("?") < 0 ? "?" : "&") +
				encodeURIComponent(args.callbackKey || "callback") + "=" +
				encodeURIComponent(callbackName)

			modules.request(script_src, {
				deserialize: function(response){
					try{
						return JSON.parse(response.replace(new RegExp('^' + callbackName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\('), '').replace(/\)$/, ''))
					}
					catch(e){
						reject(e)
					}
		...
		//=======
		// makeRequest(function(url, args, resolve, reject) {
